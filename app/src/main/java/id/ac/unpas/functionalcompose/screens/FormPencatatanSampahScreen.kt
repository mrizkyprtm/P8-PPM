...
@Composable
fun FormPencatatanSampahScreen(
    ...
    Column(
        ...
        OutlinedTextField(label = { Text(text = "Tanggal") },
            value = tanggal.value,
            onValueChange = {
                tanggal.value = it
            },
        ...
        OutlinedTextField(label = { Text(text = "Nama") }, value = nama.value, onValueChange = {
            nama.value = it
        }, modifier = Modifier
            .padding(4.dp)
        ...
        OutlinedTextField(label = { Text(text = "Berat") }, value = berat.value, onValueChange = {
            berat.value = it
        }, modifier = Modifier
            .padding(4.dp)
            .fillMaxWidth(), keyboardOptions = KeyboardOptions(
        ...
        Row(
            ...
            Button(modifier = Modifier.weight(5f), onClick = {
                if (id == null) {
                    scope.launch {
                        viewModel.insert(
                            tanggal.value.text, nama.value.text, berat.value.text
                        )
                    }
                } else {
                    scope.launch {
                        viewModel.update(
                            id, tanggal.value.text, nama.value.text, berat.value.text
                        )
                    }
                }
